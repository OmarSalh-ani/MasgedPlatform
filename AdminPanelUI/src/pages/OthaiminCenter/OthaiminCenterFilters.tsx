...
import { OthaiminCenterStudentNameDropdown } from '@/pages/OthaiminCenter/OthaiminCenterStudentNameDropdown'
import type { HomeFilterFormState } from '@/pages/OthaiminCenter/othaiminCenterUtils'
import { toOthaiminCenterLookupOptions } from '@/pages/OthaiminCenter/othaiminCenterUtils'
import type { HomeFilterOptions } from '@/types/home'
import { HOME_FORM_STATUS_OPTIONS } from '@/types/home'

interface OthaiminCenterFiltersProps {
...
export function OthaiminCenterFilters({
  form,
  options,
  isGirlTeacher,
  isExporting,
    ...
    <section className="rounded-xl border bg-white p-5 shadow-sm">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <Field label="اسم الطالب">
          <OthaiminCenterStudentNameDropdown value={form.studentName} onChange={(value) => update('studentName', value)} />
        </Field>
        <Field label="العمر">
          <div className="flex items-center gap-2">
            <Input type="number" value={form.ageFrom} placeholder="من" onChange={(event) => update('ageFrom', event.target.value)} />
            <span className="shrink-0 text-sm text-slate-500">إلى</span>
            <Input type="number" value={form.ageTo} placeholder="إلى" onChange={(event) => update('ageTo', event.target.value)} />
          </div>
        </Field>
        <Field label="الحلقة">
          <SearchableDropdown
            ...
            options={toOthaiminCenterLookupOptions(options?.circles, 'جميع الحلقات')}
        ...
        {isGirlTeacher ? (
          <Field label="نوع التسجيل">
            <SearchableDropdown
              ...
              options={toOthaiminCenterLookupOptions(options?.womanActivityTypes, 'جميع الأنواع')}
