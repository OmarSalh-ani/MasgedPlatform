    ...
    <div>
      ...
      <div className="space-y-6">
        <Section id="first-setup" title="الإعداد الأول ومدير النظام">
          <p>
            بعد تنصيب نسخة جديدة، أي زيارة للوحة التحكم تُحوَّل تلقائياً إلى صفحة{' '}
            <Inline>/setup</Inline> حتى يكتمل الإعداد. الصفحة مفتوحة بدون تسجيل دخول، وتُغلق نهائياً
            بمجرد حفظها.
          </p>
          <Rows
            rows={[
              ['اسم الشركة / المسجد', 'يظهر في لوحة التحكم والموقع العام والتطبيق'],
              ['اللون الرئيسي', 'صيغة #RRGGBB — يُطبَّق على كامل الواجهة'],
              ['النطاق', 'مثل customer.com بدون https:// — يمكن تعديله لاحقاً من «الإعدادات»'],
              ['الشعار', 'JPG أو PNG أو GIF أو WebP'],
              [
                'مدير النظام',
                'الاسم والبريد وكلمة المرور (٦ أحرف على الأقل) — البريد هو اسم المستخدم عند الدخول',
              ],
              ['روابط المتاجر', 'اختيارية — تظهر في الموقع العام لتحميل التطبيقين'],
            ]}
          />
          <p>
            بعد الحفظ يتم التحويل إلى <Inline>/login</Inline>. سجّل الدخول ببريد المدير وكلمة المرور
            التي أدخلتها، ثم أنشئ بقية المستخدمين من صفحة «المعلمين» مع تفعيل صلاحية إدارة
            المستخدمين لمن يحتاجها.
          </p>
          <Alert>
            إذا استعدت نسخة قاعدة بيانات فيها إعدادات سابقة فقد يكون الإعداد مكتملاً مسبقاً، وعندها
            لن تظهر صفحة الإعداد — استخدم حساب مدير موجود.
          </Alert>
        </Section>
...
WITH MOVE 'NewMasgedTeacherAPIDB' TO 'C:\\SQLData\\NewMasgedTeacherAPIDB.mdf',
     ...
     REPLACE;`}</Code>
          <p>
            بعد الاستعادة تأكد أن جدول <Inline>MasgedSettings</Inline> يحتوي صفاً واحداً بقيمة{' '}
            <Inline>SetupCompleted = 0</Inline> حتى تظهر صفحة الإعداد للعميل الجديد.
          </p>
        </Section>
...
.\\tool\\generate_store_icons.ps1 -LogoPath C:\\logos\\customer.png -BackgroundColor "#071B3A"`}</Code>
        ...
        <Section id="domain" title="تغيير النطاق">
          <p>
            حقل النطاق في «الإعدادات» يغيّر ما تعرضه المنظومة فقط. تغيير النطاق فعلياً يحتاج الخطوات
            التالية بالترتيب:
          </p>
          <Steps
            items={[
              ...
              <>
                ...
                <Inline>Deployment:Domain</Inline> و <Inline>ApiSettings:MediaBaseUrl</Inline>.
              </>,
              <>
                تحديث ملفات <Inline>.env</Inline> للواجهتين ثم <strong>إعادة بنائهما ونشرهما</strong>{' '}
                — عناوين API تُدمج داخل الحزمة وقت البناء.
              </>,
              <>تحديث النطاق من صفحة «الإعدادات» في لوحة التحكم.</>,
              <>
...
WITH INIT, COMPRESSION, CHECKSUM;`}</Code>
        ...
        <Section id="troubleshooting" title="حل المشكلات">
          <Rows
            rows={[
              [
                'صفحة الإعداد لا تظهر',
                <>
                  الإعداد مكتمل مسبقاً (<Inline>SetupCompleted = 1</Inline>) — سجّل الدخول بحساب
                  موجود
                </>,
              ],
              [
        ...
        <Section id="checklist" title="قائمة تحقق لعميل جديد">
          <Bullets
            items={[
              ...
              'إكمال صفحة الإعداد وإنشاء مدير النظام',
