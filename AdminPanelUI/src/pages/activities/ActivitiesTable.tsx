...
import { getActivitySubtitle } from '@/types/activities'
...
function getColumns(onDelete: (id: number) => void): DataTableColumn<ActivityListItem>[] {
  return [
    {
      id: 'title',
      header: 'العنوان',
    ...
    },
    {
      id: 'subtitle',
      header: 'النوع',
      accessor: (row) => getActivitySubtitle(row),
    },
    {
      id: 'actions',
      header: 'إجراءات',
...
export function ActivitiesTable({ items, emptyMessage, onDelete }: ActivitiesTableProps) {
  return (
    <DataTable
      data={items}
